...
# sleep_df <- sleep_df %>% 
#   mutate(gpa_group = case_when(cum_gpa <= 2.8 ~ "Low", 
#                                cum_gpa >= 3.9 ~ "High",
#                                cum_gpa > 2.8 & cum_gpa < 3.9 ~ "Middle"), 
#          term_group = case_when(zterm_units_zof_z <= -1 ~ "Heavy", 
#                                 zterm_units_zof_z >= 1 ~ "Light", 
#                                 zterm_units_zof_z > -1 & zterm_units_zof_z < 1 ~ "Average"))
...
sleep_df %>% na.omit() %>% 
ggplot(aes(round(cum_gpa), round(zterm_units_zof_z), fill = total_sleep_time)) + 
  geom_tile() +
  labs(
    title = "Heatmap of Total Sleep Time",
    x = "Cumulative GPA",
    y = "Term Units (Z)",
    fill = "Sleep Time (min)"
  ) +
